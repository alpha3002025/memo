...
- **hostPath**
  ...
  - **호스트OS** 내에서 특정 **Path**를 공유하는 것을 의미
  - 즉, **노드(=호스트OS가 설치된 vm 또는 머신)**의 **특정 물리적인 디렉터리**를 볼륨으로 선언
  - **노드 내에서만** 공유가 가능하다.
...
- PV : Persistent Volume
  - **'미리 준비되어 있는 적합한 볼륨'** 같은 개념
  - iscsi, nfs, configMap, cinder, awsElasticBlockStore 등과 같은 여러가지 디스크 리소스가 있을 때 이 것을 PV 로 등록해두는 개념
  - PVC(Persistent Volume Claim)를 이용해서 쿠버네티스 클러스터에 Volume 요청을 하면 PV 로 선언되어 있는 미리 준비되어 있는 적합한 볼륨을 찾는다.
  - 예를 들어 퍼블릭 클라우드가 아닌 온프레미스에서 nfs 50GB 를 PVC로 요청한다고 했을 때 현재 등록된 PV 에는 nfs 100GB, nfs 200GB, nfs 300GB 등이 있을수 있는데, 이 중 가장 적합한 볼륨은 nfs 100GB 이고, nfs100GB 볼륨이 할당되어 그 곳에 쓰기 작업을 하게 된다.
  - 퍼블릭 클라우드 환경에서는 관리자가 이렇게 미리 저장장치 등을 미리 준비해둘 필요가 없다.
  - 
